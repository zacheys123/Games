	...
	const getUserData = async (ev) => {
		...
		try {
			...
			setProf({
				...
				marital: prevData?.current?.marital || '',
				occupation: prevData?.current?.occupation,
				city: prevData?.current?.city,
				package: prevData?.current?.package,
				password: prevData?.current?.password,
				confirmpassword: prevData?.current?.password,
			});
		} catch (error) {
