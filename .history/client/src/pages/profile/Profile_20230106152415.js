...
const Profile = () => {
	...
	const [prof, setProf] = useState(() => {
		let prev = {
			...
			marital: prof_data?.marital,
			occupation: prof_data?.occupation,
			city: prof_data?.city,
			password: prof_data?.password,
			confirmpassword: prof_data?.password,
		};
		if (
			prev.username === '' ||
			prev.email === '' ||
	...
	// Get User Data
	const getUserData = async (ev, data) => {
		const baseUrl = process.env.REACT_APP_HOST;

		try {
			data = await axios.get(`${baseUrl}/user/v2/${id}`);
			setDataProfile(response.data);
			setProf({
				username: allprof.username,
				email: allprof.email,
				marital: allprof.marital || '',
				occupation: allprof.occupation,
				city: allprof.city,
				package: allprof.package,
				password: allprof.password,
				confirmpassword: allprof.password,
			});
		} catch (error) {
